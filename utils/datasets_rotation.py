...
# 直线增强
def line_enhance(img, min_line_length=15,max_line_length=300):
    # 转化为灰度图
    if len(img.shape) == 3:
    ...
    if lines is not None:
        lines = lines[:,0]
        # 对直线的长度进行筛选
        norm = np.linalg.norm((lines[:,[0,1]]-lines[:,[2,3]]), axis=1)
        mask = (norm >= min_line_length) & (norm < max_line_length)
        lines = lines[mask]
        for line in lines:
            x0 = round(line[0].item())
            y0 = round(line[1].item())
            x1 = round(line[2].item())
            y1 = round(line[3].item())
    ...
    try:
        ...
        if os.path.isfile(lb_file):
            ...
            if nl:
                ...
                # 但我们这里先不注释掉了
                # 由于VEDAI数据集中很多点坐标超出了图像范围，因此我们这里注释掉以下信息
