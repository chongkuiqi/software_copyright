...
class S2ANetHead(nn.Module):
    ...
    # def create_unshared_head_layers(self):
    #     if not self.shared_head:
    #         self.fam_reg_ls = nn.ModuleList(deepcopy(self.fam_reg_ls) for i in range(self.num_levels))
    #         self.fam_cls_ls = nn.ModuleList(deepcopy(self.fam_cls_ls) for i in range(self.num_levels))

    #         self.fam_reg_head = nn.ModuleList(deepcopy(self.fam_reg_head) for i in range(self.num_levels))
    #         self.fam_cls_head = nn.ModuleList(deepcopy(self.fam_cls_head) for i in range(self.num_levels))

    #         self.align_conv = nn.ModuleList(deepcopy(self.align_conv) for i in range(self.num_levels))

    #         if self.with_orconv:
    #             self.or_conv = nn.ModuleList(deepcopy(self.or_conv) for i in range(self.num_levels))
    #             self.or_pool = nn.ModuleList(deepcopy(self.or_pool) for i in range(self.num_levels))
    #         else:
    #             self.or_conv = nn.ModuleList(deepcopy(self.or_conv) for i in range(self.num_levels))
            
    #         self.odm_reg_ls = nn.ModuleList(deepcopy(self.odm_reg_ls) for i in range(self.num_levels))
    #         self.odm_cls_ls = nn.ModuleList(deepcopy(self.odm_cls_ls) for i in range(self.num_levels))

    #         self.odm_reg_head = nn.ModuleList(deepcopy(self.odm_reg_head) for i in range(self.num_levels))
    #         self.odm_cls_head = nn.ModuleList(deepcopy(self.odm_cls_head) for i in range(self.num_levels))
