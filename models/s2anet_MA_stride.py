...
from models.boxes import rboxes_encode_stride, rboxes_decode_stride
...
class S2ANetHead(nn.Module):
    ...
    def forward_single(self, x, featmap_stride):
        ...
        refine_anchor,_ = self.get_refine_anchors(fam_cls_pred.detach(), fam_bbox_pred.detach(), init_grid_anchors.clone(), featmap_stride)
    ...
    def get_refine_anchors(self, fam_cls_pred, fam_bbox_pred, init_grid_anchors, stride):
        '''
        ...
        # 解码得到refine_anchors
        refine_anchor = rboxes_decode_stride(
                init_grid_anchors, fam_bbox_pred, stride, is_encode_relative=self.is_encode_relative, wh_ratio_clip=1e-6)
    ...
    def compute_loss(self, p, targets):
        ...
        for level_id in range(num_level):
            ...
            fam_cls_loss_single_level, fam_reg_loss_single_level = self.compute_loss_single_level(fam_bbox_pred, fam_cls_pred, init_grid_anchors, 
                fam_pos_targets_one_level, fam_assign_gt_ids_one_level, stride, module_name="fam")
            ...
            odm_cls_loss_single_level, odm_reg_loss_single_level = self.compute_loss_single_level(odm_bbox_pred, odm_cls_pred, refine_anchors, 
                odm_pos_targets_one_level, odm_assign_gt_ids_one_level, stride, module_name="odm")
    ...
    def compute_loss_single_level(self, bbox_pred, cls_pred, anchors, pos_targets_batch, assign_gt_ids_batch, stride, module_name):
        '''
        ...
        if pos_targets_batch is not None and num_pos>0:
            ...
            if self.reg_loss_use_decoded:
                # 网络预测解码后的边界框
                pred_batch = rboxes_decode_stride(pos_anchors_batch, pos_bbox_pred_batch, stride, is_encode_relative=self.is_encode_relative)
                # gt boxes
            ...
            # 使用编码后的边界框计算回归损失，即SmoothL1Loss的要求
                ...
                # gt boxes编码后的边界框
                targets_batch = rboxes_encode_stride(pos_anchors_batch, pos_targets_batch[:,2:7], stride, is_encode_relative=self.is_encode_relative)
    ...
    def get_bboxes(self, p, module_name="odm"):
        ...
        if module_name == "odm":
            ...
            # FAM模块
            raise NotImplementedError("must be FAM")
        else:
    ...
    def get_bboxes_single_img(self, scores_levels, bbox_pred_levels, anchors_levels):
        ...
        bboxes = []
        # 逐个特征层级进行处理
        for score_level, bbox_pred_level, anchors_level, stride in zip(scores_levels, bbox_pred_levels, anchors_levels, self.featmap_strides):
            ...
            ## 边界框解码
            bboxes_level = rboxes_decode_stride(anchors_level, bbox_pred_level, stride, is_encode_relative=self.is_encode_relative)            
