...
class BiFPN(nn.Module):
    def __init__(self, in_channels=[512, 1024, 2048], out_channels=256):
        ...
        for i in range(self.num_ins):
            ...
            self.lateral_convs.append(
                nn.Sequential(
                    nn.Conv2d(self.in_channels[i], self.out_channels, kernel_size=(1,1), stride=(1,1), padding=0, bias=True),
                    nn.ReLU(inplace=True)
                )
            )
            # output连接的目的，是对up-down融合后的特征图进一步进行处理
    ...
    def forward(self, inputs):
        ...
        # 从通道数最少、尺寸最大的特征图开始
        temp0 = temporaries[0] 
        temp0_1 = self.downsample1(temp0)
        temp0_2 = self.downsample2(temp0_1)

        temp1 = temporaries[1]
        temp1_0 = F.interpolate(temp1, scale_factor=2, mode='nearest')
        temp1_2 = self.downsample3(temp1)

        temp2 = temporaries[2]
        temp2_1 = F.interpolate(temp2, scale_factor=2, mode='nearest')
        temp2_0 = F.interpolate(temp2_1, scale_factor=2, mode='nearest')

        
        temp = (temp0+temp1_0+temp2_0, temp1+temp0_1+temp2_1, temp2+temp0_2+temp1_2)

        outs = [
            out_conv(temp[i]) for i, out_conv in enumerate(self.out_convs)
        ]       
