...
from models.neck import FPN, PAN, BiFPN
...
class RetinaNetHead(nn.Module):
    ...
    def forward_single(self, x, featmap_stride):
        ...
        if self.with_conf:
            ...
            refine_anchor, index, bbox_pred_decode = self.get_refine_anchors(conf_pred.detach(), fam_bbox_pred.detach(), init_grid_anchors.clone())
    ...
    def assign_labels_fam_odm(self, p, targets):
        ...
        for batch_id in range(batch_size):
            ...
            if self.with_conf:
                ...
                odm_assign_gt_ids, max_ious = assign_labels(bbox_pred_decode_one_img, targets_one_img[:, 2:7], imgs_size=self.imgs_size)
                
                # 将正负样本分配结果，拆分为各个特征层级的分配结果
                odm_assign_gt_ids_levels = split_to_levels(odm_assign_gt_ids, num_grid_anchors_every_level)
    ...
    def compute_loss_single_level(self, bbox_pred, cls_pred, anchors, pos_targets_batch, assign_gt_ids_batch, module_name,conf_pred=None, max_ious_levels=None, index=None, odm_assign_gt_ids=None, odm_pos_targets=None):
        ...
        if self.with_alignconv:
            # pos_gt_ids_batch是所有anchors中正样本的位置
            # index是所有anchors中被选中的anchors的位置
            # 所有anchors中既是正样本、又是被选中的anchors的位置

            # print(f"cls_pred:{cls_pred.shape}")
            # print(f"pos_gt_ids_batch:{pos_gt_ids_batch.shape}")
            # print(f"index:{index.shape}")

            pos_gt_ids_batch = odm_assign_gt_ids >= 0

            pos_choosed_anchors_id = pos_gt_ids_batch & index

            # 把被选中的anchors中对应的正样本标签提取出来
            pos_choosed_anchors_targets_batch = odm_pos_targets[pos_choosed_anchors_id[pos_gt_ids_batch]]
            
            # 正样本个数
            num_pos = pos_choosed_anchors_id.sum().item()

            assert pos_choosed_anchors_targets_batch.shape[0] == num_pos

            # 如果存在目标，并且有正样本
            if num_pos>0:
                # 计算正样本的分类损失
                # 被选中的anchors正样本对应的分类预测值
                pos_cls_pred_batch = cls_pred[pos_gt_ids_batch[index]]
                # 正样本对应的分类标签值
                pos_cls_targets_batch = torch.zeros_like(pos_cls_pred_batch)
                # 正样本类别标签所在通道设置为1
                pos_cls_targets_batch[range(num_pos), pos_choosed_anchors_targets_batch[:,1].long()] = 1
            ...
            # 所有anchors中既是正样本、又是被选中的anchors的位置
            neg_choosed_anchors_id = neg_gt_ids_batch & index
            num_neg = neg_choosed_anchors_id.sum().item()
            if num_neg>0:
                # 负样本对应的分类预测值
                neg_cls_pred_batch = cls_pred[neg_gt_ids_batch[index]]
                # 负样本对应的分类标签值
                neg_cls_targets_batch = torch.zeros_like(neg_cls_pred_batch)
    ...
    def get_bboxes(self, p, module_name="fam"):
        ...
        if self.with_alignconv:
            ...
            # 筛选出单个anchors
                ...
                bbox_pred = []
                conf_choosed_pred = []
                for conf_pred_single_level, bbox_pred_single_level, init_grid_anchors_single_level, index_single_level in zip(conf_pred, fam_bbox_pred, init_grid_anchors, index_all):
                    
                    _, feat_h, feat_w, _, _ = bbox_pred_single_level.shape

                    bbox_pred.append(bbox_pred_single_level[index_single_level].reshape(batch_size, feat_h, feat_w, -1))
                    anchors.append(init_grid_anchors_single_level[None,:].repeat(batch_size, 1,1,1,1)[index_single_level].reshape(batch_size, feat_h, feat_w, -1))
            ...
            for level_id in range(num_level):
                if self.with_conf:
                    if not self.with_alignconv:
                        ...
                        score_one_img_one_level = cls_pred[level_id][batch_id].detach().reshape(-1, self.num_classes).sigmoid() * \
                            conf_choosed_pred[level_id][batch_id].detach().reshape(-1, 1).sigmoid()
                else:
                    score_one_img_one_level = cls_pred[level_id][batch_id].detach().reshape(-1, self.num_classes).sigmoid()
